...
without touching any file. By default the domain-lookup swap is also re-run
on entries whose existing `site` still looks like a raw domain (ends in a
dot + 2-3 word characters, e.g. 'blockclubchicago.org'); pass
--no-force-domain-lookup to leave those entries alone.

Usage: python3 reconcile.py [--all] [--dry] [--no-force-domain-lookup]
"""
...
def fill_sites(data, lookup, force_domain_lookup=True):
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--no-force-domain-lookup",
        dest="force_domain_lookup",
        action="store_false",
